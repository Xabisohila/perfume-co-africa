      ...
      {isOpen && (
        <>
          ...
          >
            ...
            {items.length > 0 && (
              <div className="border-t border-white/8 px-6 py-6 space-y-4 flex-shrink-0">
                ...
                {/* Primary CTA — PayFast secure checkout */}
                <motion.a
                  href="/checkout"
                  whileHover={{ scale: 1.02, y: -1 }}
                  whileTap={{ scale: 0.97 }}
                  className="btn-gold-shimmer glow-pulse w-full text-black font-inter font-bold text-base py-4 rounded-2xl flex items-center justify-center gap-2.5"
                >
                  <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                  </svg>
                  Secure Checkout — {fmt(orderTotal)}
                </motion.a>

                {/* Secondary — WhatsApp fallback */}
                <button
                  onClick={handleWhatsApp}
                  className="w-full flex items-center justify-center gap-2 text-white/30 hover:text-[#25D366] font-inter text-xs text-center transition-colors py-1"
                >
